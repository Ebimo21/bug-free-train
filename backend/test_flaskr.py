...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.new_question = {'question': 'New Question', 'answer': 'this is the answer', 'difficulty': 3, 'category': 3}
    ...
    def test_405_using_wrong_method_with_categories(self):
        """Test _____________ """
        res = self.client().post('/categories', json={"data": "This is a valid data"})
        data = json.loads(res.data)
    ...
    def test_405_if_question_creation_not_allowed(self):
        res = self.client().post("/questions/45", json={"question": "New Question", "answer": "this is the answer", "difficulty": 3, "category": 3})
        data = json.loads(res.data)
    ...
    def test_get_question_search_without_result(self):
        res = self.client().post("/questions", json={"searchTerm": "Whatsadfalkj"})
        data = json.loads(res.data)
    ...
    def test_get_quizzes_for_invalid_quiz_category(self):
        res = self.client().post("/quizzes", json={"previous_questions": [], "quiz_category": {"id": "234"}})
        data = json.loads(res.data)
